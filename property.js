document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupMobilePhotoGallery() {
        if (isMobile) {
            const photoGrid = document.querySelector('.photo-grid');
            const mainImage = document.querySelector('.photo-large img');
            const photoBtns = document.querySelector('.photos-btn');
            
            if (photoGrid && mainImage) {
                // Create mobile gallery indicators
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    // When clicking a dot, change the main image
                ...
                photoGrid.appendChild(mobileNav);
                ...
                function updateMainImage(index) {
                    // Get the property ID to find the correct images
                    const urlParams = new URLSearchParams(window.location.search);
                    const propertyId = urlParams.get('id') || '1';
                    
                    // Sample property data with images
                    const properties = {
                        '1': ['img1.jpg', 'img2.jpg', 'img3.avif', 'img4.avif', 'img5.avif'],
                        '2': ['img6.jpg', 'img7.avif', 'img8.avif', 'img9.jpg', 'img10.jpg'],
                        '3': ['img2.avif', 'img3.avif', 'img11.jpg', 'img12.jpg', 'img1.avif'],
                        // Add more as needed
                    };
                    
                    const images = properties[propertyId] || properties['1'];
                ...
                function updateActiveDot(index) {
                    const dots = mobileNav.querySelectorAll('.mobile-gallery-dot');
                    dots.forEach((dot, i) => {
                        if (i === index) {
    ...
    // Setup mobile booking experience
    function setupMobileBooking() {
        if (isMobile) {
            const mobileBookingBar = document.querySelector('.mobile-booking-bar');
            const reserveBtn = document.querySelector('.mobile-reserve-btn');
            
            if (mobileBookingBar && reserveBtn) {
                // Update the price to match the actual property price
                const priceAmount = document.querySelector('.price-amount');
                const mobilePrice = mobileBookingBar.querySelector('.mobile-price .price-amount');
                
                if (priceAmount && mobilePrice) {
                    mobilePrice.textContent = priceAmount.textContent;
                }
                
                // Handle reserve button clicks
                reserveBtn.addEventListener('click', () => {
                    const fullReserveBtn = document.querySelector('.reserve-button');
                    if (fullReserveBtn) {
                        fullReserveBtn.click();
                    } else {
                        alert('Reservation process would start here');
                    }
                });
                
                // Adjust layout for mobile booking bar
                document.body.style.paddingBottom = mobileBookingBar.offsetHeight + 'px';
            }
        }
    }
    
    // Fix scrolling to section issues on mobile
